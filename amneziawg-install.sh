...
download_package() {
    ...
    if wget --no-check-certificate -q -O "$awg_dir/$preferred_file" "$preferred_url" && [ -s "$awg_dir/$preferred_file" ]; then
        echo "$preferred_file"
        return 0
    fi
    ...
    if wget --no-check-certificate -q -O "$awg_dir/$fallback_file" "$fallback_url" && [ -s "$awg_dir/$fallback_file" ]; then
        echo "$fallback_file"
        return 0
    fi
...
install_awg_packages() {
    ...
    # Для SNAPSHOT используем фиксированный тег
    AWG_TAG="vSNAPSHOT"
    PKGPOSTFIX_BASE="_${AWG_TAG}_${PKGARCH}_${TARGET}_${SUBTARGET}"
    BASE_URL="https://github.com/guglovich/awg-openwrt-sf21-snapshot/releases/download/${AWG_TAG}/"
